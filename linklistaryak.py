...
class TicketBookingSystem:
    ...
    def book_ticket(self, seat_number, passenger_name):
        new_ticket = Ticket(seat_number, passenger_name)
    ...
    def display_tickets(self):
        current = self.head
        while current:
            print(f"Seat Number: {current.data.seat_number}, Passenger Name: {current.data.passenger_name}")
            current = current.next
...
def main():
    ...
    while True:
        ...
        if choice == "1":
            ...
            ticket_system.book_ticket(seat_number, passenger_name)
